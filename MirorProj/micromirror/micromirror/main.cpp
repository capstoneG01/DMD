...
void menuPause(void);
void menuStart(void);
void menuStandbyMode(void);
void menuNormalMode(void);
...
void menuOpen()
{
	...
	if (USB_IsConnected() == false)
		printf("Possible USB Connection Failures:\n Verify that micromirror is connected and powered on!\n Verify there are no other processess using the micromirror (like T-GUI)!\n Try reconnecting the USB cable to the computer!\n Hard reset the micromirror!\n", usb_error_flag);
}

// Load Image
// TO DO: WE SHOULD MAKE THESE CONSTANTS IN OUR DEFINE>H FILE
// LCR_SetMode:
// 0 is  "Video Mode"
