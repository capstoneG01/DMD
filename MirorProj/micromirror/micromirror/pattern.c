 ...
 * @param BitDpeht Bit depth of the image
 *
 * @return Pointer to the allocated image data structure
 */
Image_t *PTN_Alloc(int Width, int Height, uint08 BitDepth)
{
    Image_t *Image;
    int LineWidth = ALIGN_BYTES_NEXT(Width * BYTES_PER_PIX, 4);
...
int PTN_Copy(Image_t *DstImg, Image_t const *SrcImg)
{
    ...
    for(y = 0; y < Height; y++)
    {
        memcpy(DstPtr, SrcPtr, Width * BYTES_PER_PIX);
...
int PTN_Crop(Image_t *Image, int StartX, int StartY, int Width, int Height)
{
	...
	{
		Image->Buffer = (uint08 *)(Image + 1);
		Image->Width = Image->LineWidth/BYTES_PER_PIX;
...
/**
 * Merges the source image bits in each pixel to the destination image pixel.
 *
 * @param DstImg Destination image
 * @param SrcImg Source image
...
int PTN_Merge(Image_t *DstImg, Image_t const *SrcImg, int BitPos, int BitDepth)
{
	...
	for(y = 0; y < Height; y++)
	{
        for(x = 0; x < Width; x += BYTES_PER_PIX)
		{
 ...
 * @param BitDpeth Number of bits to extract (1 - 8). If 0 then use the
